    ...
    <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
      <>
        <View style={styles.container}>
         {Dimensions.get("window").height >= 700 && <View style={styles.account}>
          ...
          <View style={styles.account}>
            <Text
              style={{
                ...
                marginBottom: !!selectedContact ? Dimensions.get("window").width / 15 : 15,
              }}
            >
              Send money to:
            </Text>
            {!selectedContact && <Text style={{ fontFamily: 'lato-regular', color: colors.secondary, fontSize: Dimensions.get("window").width / 22 }}>{selectedPhoneOrEmail}</Text>}
            {!!selectedContact && (
              <View
                style={{
                  flexDirection: "row",
          ...
          <Text style={{ color: colors.secondary, paddingHorizontal: Dimensions.get("window").width / 15, marginTop: 10 }} >
            You will be charged <Text style={{ color: colors.primary }}>₦0.00</Text> for this transaction
          </Text>
        </View>
          ...
          <CustomButton2
            ...
            disabled={
              [!combinedValidators.phoneAndEmail(
                sanitizePhoneNumber(selectedContact?.phoneNumbers?.[0]?.number || selectedPhoneOrEmail)
              ),
              Number(amount) < 100,
            ].includes(true)
            }
